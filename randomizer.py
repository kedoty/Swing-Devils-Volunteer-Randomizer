...
class VolunteerPositions:
    ...
    def __init__(
        ...
        self.thursdays = [
            first_of_month + datetime.timedelta(n)
            for n in range(num_days)
            if (first_of_month + datetime.timedelta(n)).weekday() == THURSDAY_WEEKDAY
        ]
        ...
        self.week_list = copy.deepcopy(volunteered)
    ...
    def assign_name(self, position: str, week_num: int) -> None:
        ...
        if position not in self.week_list[week_num]:
            ...
            name = random.choice(self.duty_dict[position])
    ...
    def find_people(self) -> None:
        ...
        shuffled_week_num = random.sample(self.week_num_list, k=len(self.week_num_list))
        for week_num in shuffled_week_num:
            self.update_duty_dict(week_num)

            shuffled_duties = random.sample(
