...
#gone = {"Doty": [2,4], "Geoff": [1]}
gone = {"Mariah": [2], "Madeline": [0,3,4], "Courtney": [3,4]}
...
day = 3
friday = 11
extra_open = -1#2 #third thursday needs an extra opener
#monday = 26
...
weekly_schedule = pd.read_csv("swing_devils.csv",header=None,keep_default_na=False)

########
#get volunteer data
########
volunteer = pd.read_csv('swing_devils_vol.csv',header=0,index_col=0,keep_default_na=False)
...
def find_name(position, this_week, week_num, find_name_input, not_allowed):
    '''
    ...
    if position in week_list[week_num]:
        ...
        if counter[0] > 100:
            print("Week %d: %s at %s" %(week_num, name, position))
            raise CounterOverflowError('counter = %d' %(counter[0]))
            
        if vol_num[name] >= duty_dict['Max weeks per month'][name]:
            #print("%s at %s" %(name, position))
            raise CounterOverflowError('counter = %d' %(counter[0]))
        ...
        while (vol_num[name] >= duty_dict['Max weeks per month'][name]  #exceed weeks/month
              ) or (this_week[name] >= 2                                #exceed times/week
              ) or (name in gone and week_num in gone[name]             #person gone this week
              ) or (name in not_allowed):                               #person not allowed based 
...
def find_people():
    ...
    '''
    #initialize data
    vol_dict = {}       #dictionary containing dictionaries per person of what they can do
    duty_dict = {}      #dictionary containing a list of who can do each item
    vol_num = {}        #dictionary containing the number of time a person did something
    name_list = []      #list containing name of every person
    ...
    for week_num in range(len(thurs)):        
        ...
        # add opener
        not_allowed = []
        opener = find_name('Opening', this_week, week_num, find_name_input, not_allowed)

        # add extra opener
        if week_num == extra_open:
            not_allowed = [opener]
            find_name('Extra Opener', this_week, week_num, find_name_input, not_allowed)
        
        # add Teaching (lead)
        lead = find_name('Teaching (lead)', this_week, week_num, find_name_input, not_allowed)
        
        # add Teaching (follow)
        not_allowed = lead
        follow = find_name('Teaching (follow)', this_week, week_num, find_name_input, not_allowed)
        ...
        # add DJ
        not_allowed = [opener]
        DJ = find_name('DJ', this_week, week_num, find_name_input, not_allowed)
        
        # add First Door Shift
        not_allowed = [lead, follow, DJ]
        door = find_name('First Door Shift', this_week, week_num, find_name_input, not_allowed)
    
        # add Closing (tear down)
        not_allowed =[opener, door]
        close = find_name('Closing (tear down)', this_week, week_num, find_name_input, not_allowed)
    
        # add Closing (count till)
        not_allowed = [close, opener, door]
        find_name('Closing (count till)', this_week, week_num, find_name_input, not_allowed)

        # add Facebook person
        if week_num == 3:
            not_allowed = []
            find_name('Facebook Events', this_week, week_num, find_name_input, not_allowed)
...
for week_num,week in enumerate(week_list):
    ...
    # add the date
    weekly_schedule[col][row] = '{:%m/%d/%Y}'.format(thurs[week_num])
    ...
    for position, name in week.items():
        if position == 'Opening':
            ...
            row_add = 16
        elif (position == 'Teaching (lead)') or (position == 'Teaching (follow)'):
            continue
...
out_name = "swing_devils_out_{}_{:02}.csv".format(year,month)
